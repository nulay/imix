...
var pgl={
    ...
    wait:3000,
    mwait:100,
    masspr:[250,350,100,250,350,500,5000,10000,10,10,10],
    pik:0,
    puk:0,
...
//        this.wrlog('->.');
        if(this.isInteres(mr)){
            ...
            if($('[name="b'+mr[3]+'"]').attr('disabled')==null){
				this.wrlog('Ставим '+mr[2]);
                $('[name="b'+mr[3]+'"]').click().attr('disabled','disabled');				
            }else{
			    this.wrlog('Обновляемся...');
                $('.panelRounded_body a.button-pro').click();				
			}			
            setTimeout(function(){thisEl._startTorg(mr)},this.mwait);
        }else{
...
//            this.wrlog('->....');
    ...
    delLot:function(mr){
        this.wrlog('Закончили торги за '+(mr[0]+1)+' за '+mr[2]);
